import streamlit as st
from serpapi import GoogleSearch
import tempfile
import os

st.set_page_config(page_title="探し物は何ですか？", page_icon="🔍", layout="wide")

# APIキー取得
try:
    SERPAPI_KEY = st.secrets["SERPAPI_KEY"]
except Exception:
    st.error("⚠️ APIキーが設定されていません。")
    st.stop()

st.title("🔍 探し物は何ですか？")

with st.sidebar:
    st.header("検索条件")
    uploaded_files = st.file_uploader("画像をアップロード (最大3枚)", accept_multiple_files=True, type=['png', 'jpg', 'jpeg'], key="uploader")
    part_number = st.text_input("品番", key="pn")
    maker = st.text_input("メーカー名", key="mk")
    keywords = st.text_input("キーワード", key="kw")
    search_btn = st.button("この条件で探す", type="primary", key="btn")

def search_by_lens(file):
    """Google Lensで画像検索を実行"""
    with tempfile.NamedTemporaryFile(delete=False, suffix=".jpg") as tmp:
        tmp.write(file.getvalue())
        tmp_path = tmp.name
    
    try:
        search = GoogleSearch({
            "engine": "google_lens",
            "file": tmp_path,
            "api_key": SERPAPI_KEY,
            "hl": "ja"
        })
        res = search.get_dict()
        matches = res.get("visual_matches", [])
        if matches:
            item = matches[0] # 最も似ているものを1つ返す
            return {
                "title": item.get("title", "名称不明"),
                "price": item.get("price", {}).get("extracted", "価格不明"),
                "source": item.get("source", "不明"),
                "link": item.get("link"),
                "thumbnail": item.get("thumbnail")
            }
    except Exception as e:
        st.error(f"画像解析エラー: {e}")
    finally:
        if os.path.exists(tmp_path):
            os.remove(tmp_path)
    return None

def search_by_text(q):
    """Google Shoppingでテキスト検索を実行"""
    try:
        search = GoogleSearch({
            "engine": "google_shopping",
            "q": q,
            "api_key": SERPAPI_KEY,
            "google_domain": "google.co.jp",
            "hl": "ja",
            "gl": "jp"
        })
        res = search.get_dict()
        items = res.get("shopping_results", [])
        if items:
            item = items[0]
            return {
                "title": item.get("title", "名称不明"),
                "price": item.get("price", "価格不明"),
                "source": item.get("source", "不明"),
                "link": item.get("link"),
                "thumbnail": item.get("thumbnail")
            }
    except Exception:
        pass
    return None

if search_btn:
    results = []

    # 1. 画像があれば優先的に処理（最大3枚分）
    if uploaded_files:
        with st.spinner("画像を1枚ずつ解析中..."):
            for f in uploaded_files[:3]:
                res = search_by_lens(f)
                if res:
                    results.append(res)

    # 2. 結果が3つに満たない場合、テキストで補完
    if len(results) < 3:
        query_list = []
        if maker and part_number: query_list.append(f"{maker} {part_number}")
        if part_number: query_list.append(part_number)
        if keywords: query_list.append(keywords)
        
        for q in query_list:
            if len(results) >= 3: break
            with st.spinner(f"「{q}」で補完検索中..."):
                res = search_by_text(q)
                if res:
                    results.append(res)

    # 表示処理
    if not results:
        st.error("商品が見つかりませんでした。別の画像やキーワードを試してください。")
    else:
        st.success(f"該当しそうな商品を{len(results)}件表示します")
        st.divider()
        cols = st.columns(3)
        for i in range(len(results)):
            with cols[i]:
                item = results[i]
                with st.container(border=True):
                    if item.get("thumbnail"):
                        st.image(item["thumbnail"], use_container_width=True)
                    st.markdown(f"**{item['title'][:40]}...**")
                    st.write(f"💰 {item['price']}")
                    st.caption(f"🏬 {item['source']}")
                    st.link_button("商品を見る", item["link"], use_container_width=True)
